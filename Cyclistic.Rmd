...
Last 12 months (from March 2023 to February 2024) of Cyclistic’s historical trip data has been collected on a monthly basis in CSV files with the format **`YYYY_MM_tripdata.csv`**, and saved in a common folder.
...
#### 🔁 Running This Notebook without the source data
...
```{r load-reduced-data, eval=FALSE, include=FALSE}
library(here)
...
```{r write reduced data, eval=FALSE, warning=FALSE, include=FALSE}
# Write to CSV for use without the source raw csv files
write.csv(data, file = "reduced_data.csv", row.names = FALSE)
```
...
Next, the data frame has been checked for missing values:
```{r}
...
Exploring columns in the original dataset **`combined_data`**: <span style="color: blue;">**`start_station_name`**</span>,
...
1. Casual users have a 28% lower total ride count, but their average ride duration is 2.2x longer than members.
...
``` 
2. Electric bikes are slightly more popular than classic bikes for casual users, while the opposite is true for users with annual membership.
3. Casual users use *cyclistic* bikes with more frequency during the weekends, but still it is less than users with a member ship.
4. Regardless the day of the week, casual users ride in average longer than members. The ride duration is higher during the weekends.
5. Average ride length for Member users during week days is constant (close to 13.13 minute overall average), suggesting that members use the bikes as a mean of transportation to ride fixed distances, for instance committing to work or school.
6. Spring and Summer are the seasons with most number of rides for both types of users.
